...
const Summarizer = () => {
  ...
  const handleGenerateResponse = async () => {
    ...
    try {
      // Adjust the Axios POST request URL to your backend endpoint
      const res = await axios.post('http://localhost:4000/api/quickdash/dash-summarize', { videoUrl });
      const formattedResponse = formatResponse(res.data.summary); // Format the response summary
      setResponse(formattedResponse); // Set the response summary
    } catch (err) {
  ...
  // Function to format response text with indentation, headings, and lists
  ...
  return (
    <div className="flex flex-col items-center p-5">
      <h1 className="text-5xl font-bold mb-10 text-white">YouTube Video Summarizer</h1>
